...
class TestLoggingOAuth2LoginView:
    def test_dispatch(self, rf, mocker):
        mocker.patch("errors_db.multisalesforce.views.OAuth2LoginView.dispatch")
        logger = mocker.patch("errors_db.multisalesforce.views.logger.info")
        request = rf.get("/")
        request.session = {"socialaccount_state": (None, "some-verifier")}
...
class TestLoggingOAuth2CallbackView:
    def test_dispatch(self, rf, mocker):
        mocker.patch("errors_db.multisalesforce.views.OAuth2CallbackView.dispatch")
        logger = mocker.patch("errors_db.multisalesforce.views.logger.info")
        request = rf.get("/")
